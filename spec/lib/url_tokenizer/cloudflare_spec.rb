...
describe UrlTokenizer::Cloudflare do
  ...
  it "adds verify parameter" do
    expect(subject.call url).to include 'verify='
  end
  ...
  describe 'when url have provider parameters' do
    let(:url) { url_with_params verify: '0000000000_e5e9fa1ba31ecd1ae84f75caaa474f3a663f05f4' }

    it 'ignores and overwrites them' do
      expect(subject.call url, expires_in: 10).not_to include '0000000000_'
      expect(subject.call url, expires_in: 10).to match /verify=\d{10,}_/
    end
  end
  ...
  describe 'when requesting verify for domain, not for url' do
    it 'returns nil' do
  ...
  def url_with_params(**params)
    ...
    "http://token.livehd.example.com/hdliveedge00/test.stream/playlist.m3u8#{ query_string }"
  end
end
